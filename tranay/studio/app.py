...
def add_dataurl():
    ...
    if url.startswith("postgresql://"):
        ...
        app.config['state']['sources'][f'{name}-mongodb'] = {
            'source_type': 'mongodb',
            'url': url,
            'active': True,
        }
...
def create_new_chat():
    ...
    agent = agent_wrapper.Agent(
        ...
        tools = tranayTools(get_active_sources()).tools,
...
def new_chat_page():
    """
    Displays the new conversation page.
    """
    return boost(
        render_template('new_conversation.html'),
        retarget='#main',
        reswap='innerHTML',
        push_url='/',
    )
